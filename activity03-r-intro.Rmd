...
**Response**: no comma, forgot what I did with upper_case, and needed forward slash for "\""
...
**Response**: arg 3 is not the same length as the rest, **cbind isnt a vector 
...
**Response**: dataframe might be data.frame? and Symbol doesn't exist, it should be the other way around. Nope I was wrong, need to change = to <- and put quotation marks around Symbol.
...
**Response**: [1]1 its a data frame not a vector so the length comes back as one, need to find the number of rows with nrow
...
**Response**: I don't understand how this creates a new variable. I thought __$__ syntax was for existing varibles. 
...
**Response**: I did it after the solutions were released. It would have taken me a long time to do it on my own. I still get confused on the difference between a vector and a matrix and when to use them. I really liked the examples of manipulating the data with the brackets. It was a good refresher becuase I used R in intro to data mining. If I didnt take that class I would be really confused. 
